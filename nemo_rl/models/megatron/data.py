    ...
    with ctx:
        ...
        if pack_sequences:
            ...
            if routed_experts is not None and r3_trace_verify_forward_enabled():
                token_identity = _make_r3_trace_token_identity(input_ids, seq_lengths)
            ...
            # routed_experts and the R3 trace token identity ride the SAME per-seq
            # zigzag CP sharding as input_ids, re-derived from cu_seqlens_padded.
...
def _shard_routed_experts_for_cp(
    ...
    token_identity: Optional[torch.Tensor],  # [B, S, 3] or None (R3 forward verifier, debug)
    ...
    routed_packed = (
        torch.cat(all_routed, dim=0).unsqueeze(0) if routed_experts is not None else None
    )
    routed_cp_sharded = (
